...
import { d as pathD, frame, glyph, sc, type FrameOpts } from './primitives.js'
...
export function ptGlobal(r: Solution, g: Solution | null): SVGSVGElement {
  const o: FrameOpts = {
    w: 470,
    h: 330,
    ml: 62,
  ...
  c.push(h('path', { d: pathD(sat, S), fill: 'none', stroke: COL.sat, strokeWidth: 2.6 }))

  c.push(
    h('circle', { cx: S.X(1), cy: S.Y(1), r: 3.4, fill: '#faf8f4', stroke: COL.ink, strokeWidth: 1.6 }),
  )
  c.push(
    h(
      'text',
  ...
  c.push(
    h(
      'text',
      { x: S.X(0.55), y: S.Y(0.42), fill: COL.liq, fontSize: 12, fontFamily: 'IBM Plex Mono, monospace' },
      'liquid',
    ),
  )
  c.push(
    h(
      'text',
      { x: S.X(0.84), y: S.Y(3.2e-3), fill: COL.vap, fontSize: 12, fontFamily: 'IBM Plex Mono, monospace' },
      'vapour',
    ),
  )
  ...
  if (g) {
    ...
    c.push(h('circle', { cx: S.X(g.t8), cy: S.Y(g.P78), r: 3.2, fill: COL.ghost }))
  ...
  c.push(glyph('s', S.X(r.t8), S.Y(r.P78), false))
  c.push(
    h(
      'text',
      {
  ...
  return figure(
    { viewBox: '0 0 ' + o.w + ' ' + o.h, label: 'Global pressure-temperature plane with the cycle extent' },
    c,
  )
}
